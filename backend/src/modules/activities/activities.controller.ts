...
export class ActivitiesController {
  ...
  async getActivity(@Param('id') id: string, @Req() req: Request, @Query('includeRoute') includeRoute?: string) {
    ...
    await this.ensureCanViewActivity({ viewerId, activityId: id });
    ...
    // Attach photos (kept small so the endpoint stays fast).
    const photos = await this.prisma.activityMedia.findMany({
      where: { activityId: id, kind: 'photo', publicUrl: { not: null } },
      orderBy: { createdAt: 'asc' },
      take: 12,
    ...
    return {
      activity: {
        ...activity,
        photoUrls: photos.map((p) => p.publicUrl).filter(Boolean) as string[],
      },
    };
  }

  @Get(':id([0-9a-fA-F-]{36})/media')
  @UseGuards(OptionalSupabaseAuthGuard)
  async listActivityMedia(@Param('id') id: string, @Req() req: Request) {
    const viewerId = (req as any)?.user?.userId as string | undefined;
    await this.ensureCanViewActivity({ viewerId, activityId: id });
    ...
    return {
      items: media
        .filter((m) => Boolean(m.publicUrl))
        .map((m) => ({
          id: m.id,
          url: m.publicUrl as string,
